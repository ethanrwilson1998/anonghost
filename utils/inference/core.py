...
from utils.inference.anonymization import anonymize
...
def model_anonymize(
    ...
    for source_curr in source:
        source_curr = normalize_and_torch(source_curr)
        # apply anonymization before storing
        emb = netArc(
            F.interpolate(
                source_curr, scale_factor=0.5, mode="bilinear", align_corners=True
            )
        )
        emb = anonymize(emb, epsilon=epsilon, theta=theta)
        source_embeds.append(emb)
