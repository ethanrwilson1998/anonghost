...
def crop_face(image_full: np.ndarray, app: Callable, crop_size: int) -> np.ndarray:
    ...
    try:
        ...
        align_img = cv2.warpAffine(
            image_full, M, (crop_size, crop_size), borderValue=0.0
        )
        return [align_img]
    except:
        return [cv2.resize(image_full, (crop_size, crop_size))]
...
def get_final_image(
    ...
    for i in range(len(final_frames)):
        ...
        try:
            ...
            final = cv2.resize(frame, (full_frame.shape[1], full_frame.shape[0]))
